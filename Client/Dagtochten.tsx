...
export class DagtochtenComponent extends React.Component<DagtochtenComponentProps, DagtochtenComponentState>{
    ...
    render() {
        ...
        if (this.state.kind == "dagtochtPagina") {
            let categoryView = function (category: Types.Category_Dagtocht) : JSX.Element {
                return <div>
                    <h2>{category.title}</h2>
                    <p>{category.description}</p>
                </div>
            }
            let dagtochtView = function (dagtocht: Types.Dagtocht) : JSX.Element {
                return <div>
                    <h2> {dagtocht.name}</h2>
            ...
            function FormatDagtochten(dagtochten : Types.Dagtocht[]) : JSX.Element {
                return  <div>
                            {dagtochten.map(dagtocht =>
                                dagtochtView(dagtocht))}
                        </div>
            }
            function ordenDagtochten(dagtochten : Types.Dagtocht[], categories : Types.Category_Dagtocht[]) : JSX.Element {
                return  <div>
            ...
            let x = localStorage.getItem('favoriteDagtocht') == null ? false : true
            return  <div>
