...
export function scene_to_page<T>(can_edit:() => boolean, renderer:Renderer<T>,
  ...
  return {
    render: (slug:string) => (is_breadcrumb:boolean) => (is_animating:boolean) => (pages_count:number, logic_frame:number, force_reload:(callback?:()=>void) => void) => () =>
        (shown_relation:string, set_shown_relation:(new_shown_relation:string, callback?:()=>void)=>void) =>
        (authentication_menu:() => JSX.Element, breadcrumbs:() => JSX.Element) =>
        (nested_entity_names:Immutable.Stack<string>) =>
...
export type PageRenderer = (slug:string) => (is_breadcrumb:boolean) => (is_animating:boolean) => (pages_count:number, logic_frame:number, force_reload:(callback?:()=>void) => void) => () =>
  (shown_relation:string, set_shown_relation:(new_shown_relation:string, callback?:()=>void)=>void) =>
  (authentication_menu:() => JSX.Element, breadcrumbs:() => JSX.Element) =>
  (nested_entity_names:Immutable.Stack<string>) =>
...
export type PageManagerProps = { initial_page: Page, slug:string,  }
...
export async function render_page_manager(slug:string, target_element_id:string, initial_page:Page, ) {
  ...
  i18next.init({
    ...
    ReactDOM.render(
      <PageManager initial_page={initial_page} slug={slug}  />,
